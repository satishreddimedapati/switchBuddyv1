...
export async function generateLearningRoadmap(input: RoadmapGenerationInput): Promise<RoadmapGenerationOutput> {
  ...
  let finalPrompt = `You are an expert curriculum designer for a tech learning platform. Your task is to generate a detailed, week-by-week learning roadmap based on the user's inputs.
...
`;
  ...
  const output = result.output as RoadmapGenerationOutput;
